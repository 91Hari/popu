  ...
  const { rows } = await pool.query(
    `WITH da AS (
     ...
     SELECT co.id, co.status, co.subtotal, co.delivery_confirmation_code, co.rider_id,
            ...
            -- Delivery address: order snapshot first, fall back to customer's default saved address
            COALESCE(mo.delivery_house_no, da.house_no)  AS delivery_house_no,
            COALESCE(mo.delivery_street,   da.street)    AS delivery_street,
            COALESCE(mo.delivery_landmark, da.landmark)  AS delivery_landmark,
            COALESCE(mo.delivery_city,     da.city)      AS delivery_city,
            COALESCE(mo.delivery_state,    da.state)     AS delivery_state,
            COALESCE(mo.delivery_pincode,  da.pincode)   AS delivery_pincode,
