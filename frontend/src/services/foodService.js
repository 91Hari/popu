...
export default {
  ...
  async searchFoodsFull({ foodName, category, food_category, catererName, minPrice, maxPrice, available, customerLat, customerLng } = {}) {
    const params = new URLSearchParams();
    if (foodName)            params.set("foodName",       foodName);
    if (category)            params.set("category",       category);
    if (food_category)       params.set("food_category",  food_category);
  ...
  async getCustomerFoods({ customerLat, customerLng, limit, food_category } = {}) {
    const params = new URLSearchParams();
    if (customerLat != null) params.set("customer_lat",  String(customerLat));
    if (customerLng != null) params.set("customer_lng",  String(customerLng));
